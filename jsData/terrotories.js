document.addEventListener('DOMContentLoaded', (event) => {

    ////////////////////////////////
    //
    //  Fractions:
    //  Neutral
    //  Bandits
    //  Redania
    //  Baron
    //  Temeria
    //  Nilf
    //
    ////////////////////////////////

    addTerritory(1768 , 1857, 150, 'Redania', 'Оксенфурт');
    addTerritory(1625 , 2249, 30, 'Temeria', 'Деревня Карстен');
    addTerritory(673 , 1292, 90, 'Baron', 'Вроницы');
    addTerritory(1398 , 1805, 30, 'Redania', 'Каменоломня Старые Хрычи');
    addTerritory(465 , 1582, 55, 'Baron', 'Деревня Вересковка');
    addTerritory(57 , 967, 30, 'Nilf', 'Лесопилка Брука');
    addTerritory(2818 , 664, 60, 'Temeria', 'Мельница Белых Садов');
    addTerritory(847 , 1165, 40, 'Redania', 'Лагерь поддержки');
    addTerritory(1870 , 136, 120, 'Nilf', 'Вызима');
    addTerritory(1171 , 1620, 40, 'Bandits', 'Деревня Яворник');
    addTerritory(1055 , 1941, 45, 'Redania', 'Приграничный пост');
    addTerritory(1558 , 1080, 30, 'Redania', 'Поместье Реордан');
    addTerritory(1083 , 1255, 50, 'Redania', 'Деревня Залипье');
    addTerritory(2772 , 469, 70, 'Nilf', 'Белые Сады');
    addTerritory(2921 , 497, 30, 'Nilf', 'Белые Сады');
    addTerritory(2242 , 524, 80, 'Nilf', 'Деревня Ольшина');
    addTerritory(2119 , 581, 30, 'Nilf', 'Деревня Ольшина');
    addTerritory(2550 , 750, 30, 'Nilf', 'Руины замка Реда');
    addTerritory(2234 , 2970, 140, 'Temeria', 'Залесье');
    addTerritory(3113 , 416, 50, 'Neutral', 'Сгоревшая деревня');
    addTerritory(1775 , 1247, 40, 'Neutral', 'Деревня Тодерас');
    addTerritory(324 , 400, 40, 'Neutral', 'Деревня Хель');
    addTerritory(239 , 1020, 40, 'Neutral', 'Деревня Подлесье');
    addTerritory(889 , 1595, 30, 'Neutral', 'Корчма на перепутье');
    addTerritory(599 , 1040, 30, 'Neutral', 'Деревня Глинник');
    addTerritory(631 , 879, 50, 'Neutral', 'Деревня Рудники');
    addTerritory(1721 , 2829, 25, 'Neutral', 'Хуторок');
    addTerritory(1580 , 2780, 40, 'Neutral', 'Усадьба Мольдави');
    addTerritory(1922 , 2939, 25, 'Neutral', 'Ферма');
    addTerritory(1914 , 2855, 25, 'Neutral', 'Хутор Фолье');
    addTerritory(1862 , 3022, 50, 'Temeria', 'Деревня Лесьна');
    addTerritory(1861 , 2755, 50, 'Temeria', 'Хозяйство Фолье');
    addTerritory(2435 , 2645, 40, 'Temeria', 'Наследие Предков');


});
